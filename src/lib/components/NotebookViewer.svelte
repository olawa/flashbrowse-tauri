...
<div class="flex flex-col h-full bg-[var(--bg-surface)] text-[var(--text-primary)] select-text overflow-hidden font-sans">
  <!-- Notebook Header -->
  <div class="flex items-center justify-between px-4 py-2 bg-[var(--bg-panel)] border-b border-[var(--border)] shrink-0 text-xs select-none">
    <div class="flex items-center gap-2">
      ...
      <span class="text-[var(--text-secondary)] text-[11px] font-mono">
  ...
  <div class="flex-1 overflow-auto p-4 space-y-4 max-w-5xl mx-auto w-full">
    ...
    {:else if cells.length === 0}
      <div class="text-center text-[var(--text-muted)] py-12 text-xs">
    ...
    {:else}
      {#each cells as cell, cIdx}
        ...
        {#if cell.cell_type === 'markdown'}
          <!-- Markdown Cell -->
          <div class="p-4 rounded-lg bg-[var(--bg-panel)]/80 border border-[var(--border)] text-sm leading-relaxed space-y-2">
            {@html renderMarkdown(src)}
          </div>

        {:else if cell.cell_type === 'code'}
          <!-- Code Cell -->
          <div class="rounded-lg bg-[var(--bg-base)] border border-[var(--border)] overflow-hidden shadow-md">
            <!-- Code Input -->
            <div class="flex items-start bg-[var(--bg-surface)] border-b border-[var(--border)]">
              <!-- Execution Prompt -->
              <div class="w-16 py-2.5 text-right pr-3 font-mono text-[11px] font-bold text-cyan-400 select-none shrink-0 bg-[var(--bg-surface)]/60">
                [{cell.execution_count ?? ' '}] :
              </div>
              <!-- Code Text -->
              <pre class="flex-1 p-2.5 m-0 font-mono text-xs text-[var(--text-primary)] overflow-x-auto whitespace-pre leading-relaxed">{src}</pre>
            </div>

            <!-- Code Output (if any) -->
            {#if cell.outputs && cell.outputs.length > 0}
              <div class="divide-y divide-[#202636]/60 bg-[var(--bg-base)]">
                {#each cell.outputs as out}
                  <!-- 1. Text Stream (stdout / stderr) -->
                  {#if out.text}
                    <div class="flex items-start">
                      <div class="w-16 py-1.5 text-right pr-3 font-mono text-[10px] text-[var(--text-muted)] select-none shrink-0">
                        {out.name === 'stderr' ? 'err:' : 'out:'}
                      </div>
                      <pre class="flex-1 p-2 m-0 font-mono text-[11px] {out.name === 'stderr' ? 'text-rose-400' : 'text-[var(--text-primary)]'} whitespace-pre-wrap">{getOutputText(out.text)}</pre>
                    </div>
                  {/if}
                  ...
                  {#if out.data}
                    ...
                    {#if out.data['image/png']}
                      <div class="flex items-start p-3 bg-white/[0.02]">
                        <div class="w-16 text-right pr-3 font-mono text-[10px] text-[var(--text-muted)] select-none shrink-0">
                          plot:
                        </div>
                    ...
                    {#if out.data['text/html']}
                      <div class="flex items-start p-3 overflow-x-auto">
                        ...
                        <div class="flex-1 text-xs select-text overflow-x-auto text-[var(--text-primary)]">
